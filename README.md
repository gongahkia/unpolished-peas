...
`zig build contentc -- <project-directory> [output-directory]` canonicalizes the manifest entry scene plus `.upassets` files under the asset root and `.upmap` files under `maps/`; unchanged source fingerprints reuse existing `.upc` artifacts. `zig build peas -- compile [project-directory] [output-directory]` provides the same project workflow.
