...
pub fn build(b: *std.Build) void {
    ...
    const exe = b.addExecutable(.{ .name = "protocol-desktop-fixture", .root_module = module });
    b.installArtifact(exe);
    const run = b.addRunArtifact(exe);
    run.setEnvironmentVariable("UP_ASSET_ROOT", b.pathFromRoot("assets"));
    if (b.args) |args| run.addArgs(args);
    const run_step = b.step("run", "Run the bounded stable-protocol desktop fixture");
    run_step.dependOn(&run.step);
    const tests = b.addTest(.{ .root_module = module });
    const run_tests = b.addRunArtifact(tests);
    const test_step = b.step("test", "Test the stable-protocol desktop fixture");
