...
pub fn build(b: *std.Build) void {
    ...
    const exe = b.addExecutable(.{ .name = "external-animation-gameplay", .root_module = module });
    ...
    const run_step = b.step("run", "Run the bounded external animation fixture");
    run_step.dependOn(&run.step);
    const tests = b.addTest(.{ .root_module = module });
    const run_tests = b.addRunArtifact(tests);
    const test_step = b.step("test", "Test the external animation fixture");
