...
const Game = struct {
    ...
    animation: up.assets.AnimationPlayer,
    blip: up.assets.Sound,
    player: up.core.Rect = .{ .x = 8, .y = 28, .w = 4, .h = 4 },
    obstacle: up.core.Rect = .{ .x = 32, .y = 20, .w = 6, .h = 24 },
    camera: up.graphics.Camera2D = .{ .position = .{ .x = 32, .y = 32 } },
    ...
    pub fn draw(self: *Game, ctx: *sdl.Context) void {
        ...
        camera.fillRect(self.obstacle, up.core.Color.rgb(115, 142, 170));
        camera.drawAtlasFrame(self.atlas.*, self.animation.frame(), .{ .x = self.player.x, .y = self.player.y }, .{ .scale = 2 });
        ctx.text("ARROWS MOVE  F3 DIAGNOSTICS", 2, 2, up.core.Color.white);
    }

    fn initResources(allocator: std.mem.Allocator) !Game {
        const atlas = try allocator.create(up.assets.Atlas);
        errdefer allocator.destroy(atlas);
        atlas.* = try initAtlas(allocator);
        errdefer atlas.deinit();
        var blip = try initSound(allocator);
        errdefer blip.deinit();
        const animation = up.assets.AnimationPlayer.init(atlas, atlas.findAnimation("walk") orelse return error.MissingAtlasAnimation);
    ...
    fn advance(self: *Game, bindings: up.input.ActionMap, input: up.input.Input, dt: f32) Move {
        const direction = up.core.Vec2{ .x = bindings.value(input, "game", "right") - bindings.value(input, "game", "left") };
...
fn diagnosticsRequested(bindings: up.input.ActionMap, input: up.input.Input) bool {
    return bindings.value(input, "game", "diagnostics") > 0;
}

fn initAtlas(allocator: std.mem.Allocator) !up.assets.Atlas {
    const pixels = try allocator.dupe(up.core.Color, &.{
        up.core.Color.rgb(255, 198, 74), up.core.Color.rgb(255, 198, 74), up.core.Color.rgb(113, 232, 162), up.core.Color.rgb(113, 232, 162),
        up.core.Color.rgb(255, 198, 74), up.core.Color.rgb(255, 198, 74), up.core.Color.rgb(113, 232, 162), up.core.Color.rgb(113, 232, 162),
    });
    errdefer allocator.free(pixels);
    const image_path = try allocator.dupe(u8, "generated-atlas");
    errdefer allocator.free(image_path);
    var frames = try allocator.alloc(up.assets.AtlasFrame, 2);
    ...
    initialized_frames += 1;
    var animations = try allocator.alloc(up.assets.Animation, 1);
    ...
    const animation_frames = try allocator.dupe(up.assets.AnimationFrame, &.{ .{ .frame = .{ .index = 0 }, .duration = 0.1 }, .{ .frame = .{ .index = 1 }, .duration = 0.1 } });
