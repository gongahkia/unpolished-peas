...
const game_mod = @import("puzzle-game");
...
pub export fn up_browser_frame(timestamp_ms: f64) void {
    ...
    var step: u32 = 0;
    while (step < timing.update_steps) : (step += 1) _ = game.step(input);
    render();
    frame_token = contract.scheduleFrame();
}
...
pub export fn up_browser_puzzle_selected() u32 {
    return @intCast(game.selected);
}

pub export fn up_browser_puzzle_lit() u32 {
    return @intCast(game.diagnostics().lit);
}

pub export fn up_browser_puzzle_render_status() i32 {
...
fn render() void {
    render_status = contract.clear(rgba(13, 18, 30));
    if (render_status != @intFromEnum(contract.Status.ok)) return;
    for (game.cells, 0..) |lit, index| {
        const x = board_x + @as(i32, @intCast(index % game_mod.columns)) * (cell_size + cell_gap);
        const y = board_y + @as(i32, @intCast(index / game_mod.columns)) * (cell_size + cell_gap);
        const selected = index == game.selected;
        submit(contract.drawRect(x - 1, y - 1, cell_size + 2, cell_size + 2, if (selected) rgba(255, 198, 74) else rgba(91, 124, 158)));
        submit(contract.drawRect(x, y, cell_size, cell_size, if (lit) rgba(113, 232, 162) else rgba(31, 47, 68)));
        if (selected) submit(contract.drawRect(x + 6, y + 6, 8, 8, rgba(255, 198, 74)));
    }
