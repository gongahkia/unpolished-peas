...
const game_mod = @import("topdown-game");
...
var paused = false;
var canvas_width: u32 = game_mod.width;
var canvas_height: u32 = game_mod.height;
...
pub export fn up_browser_frame(timestamp_ms: f64) void {
    ...
    var step: u32 = 0;
    while (step < timing.update_steps) : (step += 1) _ = game.step(input, timing.update_seconds);
    render();
    frame_token = contract.scheduleFrame();
}
...
pub export fn up_browser_topdown_player_x() f32 {
    return game.player.x;
}

pub export fn up_browser_topdown_player_y() f32 {
    return game.player.y;
}

pub export fn up_browser_topdown_render_status() i32 {
...
fn render() void {
    render_status = contract.clear(rgba(10, 18, 26));
    if (render_status != @intFromEnum(contract.Status.ok)) return;
    render_status = contract.setCamera(1, game.player.x, game.player.y, 2.5, 0, 0, 0, @floatFromInt(canvas_width), @floatFromInt(canvas_height));
    if (render_status != @intFromEnum(contract.Status.ok)) return;
    var x: i32 = 0;
    while (x <= game_mod.width) : (x += 16) submit(contract.drawRect(x, 0, 1, game_mod.height, rgba(23, 35, 47)));
    var y: i32 = 0;
    while (y <= game_mod.height) : (y += 16) submit(contract.drawRect(0, y, game_mod.width, 1, rgba(23, 35, 47)));
    submit(contract.drawRect(24, 31, 24, 12, rgba(51, 96, 122)));
    submit(contract.drawRect(108, 56, 20, 16, rgba(133, 66, 61)));
    submit(contract.drawRect(28, 38, 8, 8, rgba(255, 198, 74)));
    submit(contract.drawRect(124, 58, 8, 8, rgba(113, 232, 162)));
    submit(contract.drawRect(@intFromFloat(game.player.x + game.aim.x * 8 - 1), @intFromFloat(game.player.y + game.aim.y * 8 - 1), 3, 3, rgba(255, 198, 74)));
    submit(contract.drawRect(@intFromFloat(game.player.x - 6), @intFromFloat(game.player.y - 6), 12, 12, rgba(225, 232, 240)));
    submit(contract.setCamera(0, 0, 0, 1, 0, 0, 0, 0, 0));
