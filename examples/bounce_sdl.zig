...
const Ball = struct {
    ...
    fn update(self: *Ball, dt: f32, input: up.input.Input) void {
...
const Game = struct {
    ...
    pub fn draw(self: *Game, ctx: *sdl.Context) void {
        ...
        while (x < @as(i32, @intCast(ctx.canvas.width))) : (x += 16) {
            ctx.canvas.line(x, 0, x, @intCast(ctx.canvas.height - 1), up.core.Color.rgb(32, 39, 50));
        }
        var y: i32 = 0;
        while (y < @as(i32, @intCast(ctx.canvas.height))) : (y += 16) {
            ctx.canvas.line(0, y, @intCast(ctx.canvas.width - 1), y, up.core.Color.rgb(32, 39, 50));
        }
        ctx.canvas.strokeRect(0, 0, @intCast(ctx.canvas.width), @intCast(ctx.canvas.height), up.core.Color.rgb(91, 104, 124));
        ctx.canvas.fillCircle(@intFromFloat(self.ball.pos.x), @intFromFloat(self.ball.pos.y), self.ball.radius, up.core.Color.rgb(255, 198, 74));
        ctx.text("SDL_GPU", 4, 4, up.core.Color.rgb(225, 232, 240));
        if (ctx.down(.action)) {
            ctx.text("ACTION", 4, 76, up.core.Color.rgb(113, 232, 162));
        }
    }
};
