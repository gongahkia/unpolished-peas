...
pub fn main() !void {
    ...
    const camera = up.graphics.Camera2D{ .position = .{ .x = 80, .y = 48 }, .zoom = 1 };
    const images = [_]up.assets.Image{try assets.tryImage(player)};
    map.drawImages(up.graphics.CameraCanvas.init(&canvas, &camera), &images);
