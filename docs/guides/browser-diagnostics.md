...
Run `zig build test-browser-renderer-diagnostics` for schema checks, `zig build test-browser-chromium` for the packaged artifact path, and `zig build test-browser-renderer-parity` for the forced WebGL 2/WebGPU corpus. Parity permits an absolute per-channel RGBA delta of one; a failure retains both PNG captures, dimensions, renderer diagnostics, and command traces in `zig-out/diagnostics/browser-renderer-parity/` (or `UP_DIAGNOSTICS_ROOT`).
