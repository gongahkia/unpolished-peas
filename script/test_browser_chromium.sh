...
game=${1:-bounce}
case "$game" in bounce|topdown|puzzle|platformer) ;; *) printf '%s\n' 'usage: test_browser_chromium.sh [bounce|topdown|puzzle|platformer]' >&2; exit 64 ;; esac
case "$game" in
    platformer) input_key=d; input_index=3 ;;
    *) input_key=w; input_index=0 ;;
esac
...
cleanup() {
    ...
    if [ "$status" -ne 0 ]; then
        ...
        cp "$tmp"/*.log "$tmp"/*-artifacts.json "$artifact_dir" 2>/dev/null || true
        if [ -d "$tmp/.playwright-cli" ]; then cp -R "$tmp/.playwright-cli" "$artifact_dir/playwright" 2>/dev/null || true; fi
    fi
...
capture_artifacts() {
    ...
    "$pw" -s="$session" eval "async () => { const api = window.unpolishedPeas; await api?.captureFrame?.(); const version = navigator.userAgent.match(/(?:Firefox|Chrome)\\/([\\d.]+)/)?.[1] ?? null; const value = {game: \"$game\", browser: {name: \"$browser\", version, user_agent: navigator.userAgent}, renderer: api?.renderer ?? null, diagnostic: api?.rendererDiagnostic ?? null, artifacts: api?.host?.artifacts?.() ?? []}; const bytes = new TextEncoder().encode(JSON.stringify(value)); let text = \"\"; for (const byte of bytes) text += String.fromCharCode(byte); return btoa(text); }" > "$output"
...
persist_artifacts() {
    mkdir -p "$artifact_dir"
    cp "$tmp/$1-artifacts.json" "$artifact_dir/$1-artifacts.json"
}

cd "$repo"
script/package_web.sh "$tmp/package" --game "$game"
bundle="$tmp/package/unpolished-peas-$game-web"
python3 -m http.server "$port" --bind 127.0.0.1 --directory "$bundle" >"$tmp/server.log" 2>&1 &
server=$!
sleep 1
cd "$tmp"
"$pw" --session "$session" open "http://127.0.0.1:$port/?renderer=webgl2" --browser "$engine"
...
"$pw" --session "$session" keydown "$input_key"
"$pw" -s="$session" eval "() => { const input = window.unpolishedPeas?.host.input(); return Boolean(window.unpolishedPeas?.runtime && input?.down[$input_index] && input.pointerDown[0] && Number.isFinite(input.pointer.canvasX) && Number.isFinite(input.pointer.canvasY)); }" | grep -F true
if [ "$game" = topdown ]; then
    ...
    sleep 1
    "$pw" -s="$session" eval '() => { const runtime = window.unpolishedPeas.runtime; return runtime.up_browser_platformer_render_status() === 0 && runtime.up_browser_platformer_player_x() > 12 && Number.isFinite(runtime.up_browser_platformer_player_y()); }' | grep -F true
fi
"$pw" -s="$session" eval "() => { window.dispatchEvent(new Event(\"blur\")); const input = window.unpolishedPeas.host.input(); return !input.down[$input_index] && !input.pointerDown[0] && input.released[$input_index] && input.pointerReleased[0]; }" | grep -F true
...
"$pw" -s="$session" close
"$pw" --session "$session" open "http://127.0.0.1:$port/?renderer=webgpu" --browser "$engine"
...
if [ "$game" = topdown ]; then
    ...
    "$pw" -s="$session" eval '() => { const runtime = window.unpolishedPeas.runtime; return runtime.up_browser_platformer_render_status() === 0 && Number.isFinite(runtime.up_browser_platformer_player_x()) && Number.isFinite(runtime.up_browser_platformer_player_y()); }' | grep -F true
fi
"$pw" -s="$session" click 'canvas[data-unpolished-peas]'
"$pw" --session "$session" mousedown
"$pw" --session "$session" keydown "$input_key"
...
if [ "$game" = puzzle ]; then
    ...
    sleep 1
    "$pw" -s="$session" eval '() => window.unpolishedPeas.runtime.up_browser_platformer_player_x() > 12' | grep -F true
fi
"$pw" -s="$session" eval "() => { window.dispatchEvent(new Event(\"blur\")); const input = window.unpolishedPeas.host.input(); return !input.down[$input_index] && !input.pointerDown[0] && input.released[$input_index] && input.pointerReleased[0]; }" | grep -F true
"$pw" --session "$session" mouseup
...
if capture_artifacts webgpu; then
    persist_artifacts webgpu
else
...
"$pw" --session "$session" open "http://127.0.0.1:$port/?renderer=auto" --browser "$engine"
