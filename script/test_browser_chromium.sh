...
diagnostics=${UP_DIAGNOSTICS_ROOT:-$repo/zig-out/diagnostics/browser-chromium}
tmp=$(mktemp -d)
cleanup() {
    status=$?
    if [ "$status" -ne 0 ]; then
...
capture_artifacts() {
    ...
    "$pw" -s="$session" eval 'async () => { const api = window.unpolishedPeas; await api?.captureFrame?.(); const value = {renderer: api?.renderer ?? null, diagnostic: api?.rendererDiagnostic ?? null, artifacts: api?.host?.artifacts?.() ?? []}; const bytes = new TextEncoder().encode(JSON.stringify(value)); let text = ""; for (const byte of bytes) text += String.fromCharCode(byte); return btoa(text); }' > "$output"
...
"$pw" -s="$session" eval '() => window.unpolishedPeas?.renderer === "webgpu"' | grep -F true
