...
"$pw" -s="$session" eval '() => { const diagnostic = window.unpolishedPeas?.rendererDiagnostic; const artifact = window.unpolishedPeas?.host?.artifacts().find((value) => value.name === "renderer-diagnostics.json"); return diagnostic?.version === 1 && diagnostic.requested_renderer === "webgpu" && diagnostic.selected_renderer === null && diagnostic.fallback_reason === "forced_renderer_unsupported" && diagnostic.context_status === "not_checked" && diagnostic.capabilities.webgpu === "unsupported" && diagnostic.recovery_instruction.includes("?renderer=webgl2") && Boolean(artifact) && artifact.data === JSON.stringify(diagnostic); }' | grep -F true
"$pw" -s="$session" close
