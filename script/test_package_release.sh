...
trap 'rm -rf "$cache" "$effects_consumer" "$networking_consumer" "$physics_consumer" "$ui_consumer"' EXIT HUP INT TERM
cd "$repo"
...
rmdir "$effects_consumer" "$networking_consumer" "$physics_consumer" "$ui_consumer"
