...
while [ "$#" -gt 0 ]; do
    case "$1" in
        --game)
            shift
            [ "$#" -gt 0 ] || { printf '%s\n' 'usage: package_macos.sh [output-directory] [--game <bounce|topdown|puzzle|platformer>]' >&2; exit 64; }
            game=$1
            ;;
        *)
...
case "$game" in
    ...
    *) printf '%s\n' 'package_macos.sh: unsupported game; use bounce, topdown, puzzle, or platformer' >&2; exit 64 ;;
esac
